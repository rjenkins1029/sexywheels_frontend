...
const Cart = () => {
    ...
    const handleRemoveButtonClick = async (event) => {
        setIsLoading(true);
        const carId = Number(event.target.getAttribute('data-id'));
                    ...
                    >
                        <Box sx={{ display: 'flex', justifyContent: 'center' }}>
                            <Typography variant="h1" gutterBottom sx={{ fontWeight: 'bold'}}>
                                {cartItems=== 0 ? 'Your Cart Is Empty' : 'Your Cart'}
                            </Typography>
                        </Box>
                        {
                            cartItems !== 0 &&
                            <>
                                <List disablePadding>
                                    {
                                        setCartItems((item) => (
                                            <ListItem key={item} sx={{ py: 1, px: 0, borderBottom: '1px solid black' }}>
                                                <ListItemText primary={item} primaryTypographyProps={{ variant: 'h3' }} secondary={item} secondaryTypographyProps={{ variant: 'h4', }}/>
                                                <Box sx={{ display: 'flex', flexDirection: 'column', justifyContent: 'end' }}>
                                                    <Typography variant="h3">{item}</Typography>
                                                    <Button
                                                        data-id={token ? item : item.id}
                                                        type="button"
                                                        variant="contained"
